...
void client::readRequest(char* request, int type, int size, char* filename) {
    ...
    t->stop();
    int elapsedTime = t->getElapsedTime();
    char logMessage[256];
    //calculate speed in kpB/s
    double speed = (double)totalReceived*1000.0 / (elapsedTime*1024.0);
    sprintf(logMessage, "Received %d bytes in %d milliseconds, speed: %f kB/s", totalReceived, elapsedTime, speed);
    l->anyLog(logMessage);
...
void client::writeRequest(char* request, int type, int size, char* filename) {
    ...
    char logMessage[256];
    //calculate speed in kpB/s
    double speed = (double)totalSent*1000.0 / (elapsedTime*1024.0);
    sprintf(logMessage, "Sent %d bytes in %d milliseconds, speed: %f kB/s", totalSent, elapsedTime, speed);
    l->anyLog(logMessage);
    closesocket(clientSocket);
    WSACleanup();
}
